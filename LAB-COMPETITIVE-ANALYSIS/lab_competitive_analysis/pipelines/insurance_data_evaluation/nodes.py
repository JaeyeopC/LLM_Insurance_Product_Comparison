...
class CategorizationApp:
    ...
    def categorize(self, categorization_query):
        oai_client = OpenAI(api_key=self.openai_api_key)
        response = (
...
def categorization_evaluation(categorization_ground_truth_table: pd.DataFrame, categories_file: str, classification_prompt: str, openai_api_key: str) -> pd.DataFrame:
    ...
    categorization_app = CategorizationApp(openai_api_key=openai_api_key)
    ...
    for i, row in enumerate(golden_set):
        ...
        # Check if score is available before accessing
        similarity_score = score[0] if score else np.nan
        ground_truth_category = score[1]['ground_truth_response'] if score else "N/A"
        llm_response_category = response if response else "N/A"
